          ...
          <div className="mb-4">
            ...
            <div className="h-1.5 bg-white/15 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${progress}%`,
                  animation: 'tl-progress-fill 900ms cubic-bezier(0.23,1,0.32,1) both',
                  ['--tl-progress-to' as string]: `${progress}%`,
                }}
              >
...
function SkeletonCard(): React.ReactElement {
  return (
    ...
    >
      <div className="tl-skeleton" style={{ height: 120, borderRadius: 0 }} />
      <div style={{ padding: '12px 14px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div className="tl-skeleton" style={{ height: 14, width: '70%' }} />
        <div className="tl-skeleton" style={{ height: 11, width: '50%' }} />
      </div>
    </div>
  )
}
...
export default function DashboardPage(): React.ReactElement {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => (localStorage.getItem('traveloop_dashboard_view') as 'grid' | 'list') || 'grid')
  ...
  const toggleViewMode = () => {
    setViewMode(prev => {
      ...
      localStorage.setItem('traveloop_dashboard_view', next)
      return next
    })
  }
    ...
    <div style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: 'var(--bg-secondary)', ...font }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', overscrollBehavior: 'contain', marginTop: 'var(--nav-h)' }}>
        <div style={{ maxWidth: 1300, margin: '0 auto', paddingTop: 32, paddingLeft: 20, paddingRight: 20, paddingBottom: 'calc(100px + env(safe-area-inset-bottom, 0px))' }}>
          ...
          {isLoading && (
            <>
              <div className="tl-skeleton" style={{ height: 260, borderRadius: 24, marginBottom: 32 }} />
          ...
          {!isLoading && (viewMode === 'grid' ? rest : trips).length > 0 && (
            viewMode === 'grid' ? (
              <div className="trip-grid hidden md:grid tl-stagger" style={{ gap: 16, marginBottom: 40 }}>
                {rest.map(trip => (
                  <TripCard
                    key={trip.id}
                    trip={trip}
              ...
              <div className="hidden md:flex tl-stagger" style={{ flexDirection: 'column', gap: 8, marginBottom: 40 }}>
                {trips.map(trip => (
                  <TripListItem
                    key={trip.id}
                    trip={trip}
