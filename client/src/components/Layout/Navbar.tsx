...
import { Plane, LogOut, Settings, ChevronDown, Shield, ArrowLeft, Users, Moon, Sun, Monitor, CalendarDays, Briefcase, Globe, Compass, MapPin, FileText, Receipt } from 'lucide-react'
...
export default function Navbar({ tripTitle, tripId, onBack, showBack, onShare }: NavbarProps): React.ReactElement {
  ...
  const toggleDarkMode = () => {
    document.documentElement.classList.add('tl-theme-transitioning')
    ...
    themeTransitionTimer.current = window.setTimeout(() => {
      document.documentElement.classList.remove('tl-theme-transitioning')
      themeTransitionTimer.current = null
    }, 360)
  }
      ...
      <div className="flex items-center gap-3 min-w-0">
        {showBack && (
          <button onClick={onBack}
            className="tl-back-btn p-1.5 rounded-lg transition-colors flex items-center gap-1.5 text-sm flex-shrink-0"
            style={{ color: 'var(--text-muted)' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <ArrowLeft className="tl-back-icon w-4 h-4" />
            <span className="hidden sm:inline">{t('common.back')}</span>
          </button>
        )}

        <Link to="/dashboard" className="flex items-center gap-2 transition-colors flex-shrink-0">
          <img src={dark ? '/icons/icon-white.svg' : '/icons/icon-dark.svg'} alt="Traveloop" className="sm:hidden" style={{ height: 22, width: 22 }} />
          <span className="hidden sm:flex items-center gap-2">
            <img src={dark ? '/icons/icon-white.svg' : '/icons/icon-dark.svg'} alt="" style={{ height: 20, width: 20 }} />
            <span style={{ fontWeight: 700, fontSize: 17, letterSpacing: '-0.3px', color: dark ? '#f4f4f5' : '#111827' }}>Traveloop</span>
          </span>
        </Link>

        {/* Always-visible nav links — My Trips + Community + Notes + Invoice */}
        {!tripTitle && (
          <>
            ...
            <Link to="/community"
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors flex-shrink-0"
              style={{
                color: location.pathname === '/community' ? '#0ea5e9' : 'var(--text-muted)',
                background: location.pathname === '/community' ? 'rgba(14,165,233,0.10)' : 'transparent',
              }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(14,165,233,0.08)'}
              onMouseLeave={e => { if (location.pathname !== '/community') e.currentTarget.style.background = 'transparent' }}>
            ...
            <Link to="/notes"
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors flex-shrink-0"
              style={{
                color: location.pathname === '/notes' ? '#0ea5e9' : 'var(--text-muted)',
                background: location.pathname === '/notes' ? 'rgba(14,165,233,0.10)' : 'transparent',
              }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(14,165,233,0.08)'}
              onMouseLeave={e => { if (location.pathname !== '/notes') e.currentTarget.style.background = 'transparent' }}>
            ...
            <Link to="/invoice"
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors flex-shrink-0"
              style={{
                color: location.pathname === '/invoice' ? '#0ea5e9' : 'var(--text-muted)',
                background: location.pathname === '/invoice' ? 'rgba(14,165,233,0.10)' : 'transparent',
              }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(14,165,233,0.08)'}
              onMouseLeave={e => { if (location.pathname !== '/invoice') e.currentTarget.style.background = 'transparent' }}>
        ...
        <div className="relative">
          ...
          {userMenuOpen && ReactDOM.createPortal(
            <>
              ...
              <div className="tl-menu-enter w-52 rounded-xl shadow-xl border overflow-hidden" style={{ position: 'fixed', top: 'var(--nav-h)', right: 8, zIndex: 9999, background: 'var(--bg-card)', borderColor: 'var(--border-primary)' }}>
                  ...
                  {appVersion && (
                    <div className="px-4 pt-2 pb-2.5 text-center" style={{ marginTop: 4, borderTop: '1px solid var(--border-secondary)' }}>
                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--bg-tertiary)', borderRadius: 99, padding: '4px 12px' }}>
                        <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-faint)' }}>Traveloop v{appVersion}</span>
                      </div>
                    </div>
                  )}
                </div>
