...
export const RESERVATION_SOURCE_ID = 'tl-reservations'
export const RESERVATION_LINE_LAYER_ID = 'tl-reservations-lines'
...
function buildStatsHtml(mainLabel: string | null, subLabel: string | null): { html: string; width: number; height: number } {
  ...
  const html = `<div class="tl-stats-inner" style="
  ...
  private updateStatsRotation = () => {
    ...
    for (const entry of this.statsMarkers) {
      ...
      try {
        ...
        const inner = el.querySelector('.tl-stats-inner') as HTMLElement | null
