...
function buildLocationEl(): { root: HTMLDivElement; cone: HTMLDivElement } {
  ...
  pulse.style.cssText = `
    ...
    animation:tl-location-pulse 2s ease-out infinite;
  `
...
function ensurePulseStyle() {
  if (document.getElementById('tl-location-style')) return
  const s = document.createElement('style')
  s.id = 'tl-location-style'
  s.textContent = `
    @keyframes tl-location-pulse {
...
export function attachLocationMarker(map: mapboxgl.Map): LocationMarkerHandle {
  ...
  const ensureAccuracyLayer = () => {
    if (map.getSource('tl-location-accuracy')) return
    try {
      map.addSource('tl-location-accuracy', {
        type: 'geojson',
      ...
      map.addLayer({
        id: 'tl-location-accuracy',
        type: 'fill',
        source: 'tl-location-accuracy',
        paint: {
          'fill-color': '#3b82f6',
          'fill-opacity': 0.14,
  ...
  const setAccuracy = (p: GeoPosition) => {
    const src = map.getSource('tl-location-accuracy') as mapboxgl.GeoJSONSource | undefined
    if (!src) return
  ...
  const handle: LocationMarkerHandle = {
    ...
    destroy: () => {
      ...
      try {
        if (map.getLayer('tl-location-accuracy')) map.removeLayer('tl-location-accuracy')
        if (map.getSource('tl-location-accuracy')) map.removeSource('tl-location-accuracy')
      } catch { /* noop */ }
    },
  }
