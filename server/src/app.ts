...
import { tlOAuthProvider, tlClientsStore } from './mcp/oauthProvider';
...
export function createApp(): express.Application {
  ...
  app.use('/oauth/authorize', mcpAddonGate, authorizationHandler({ provider: tlOAuthProvider }));

  // SDK DCR handler: accepts registrations without scope (fixes issue #959 bug 2)
  app.use('/oauth/register', mcpAddonGate, clientRegistrationHandler({ clientsStore: tlClientsStore }));
