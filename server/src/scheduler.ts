...
let tlPhotoCacheTask: ScheduledTask | null = null;

function startTrekPhotoCacheCleanup(): void {
  if (tlPhotoCacheTask) { tlPhotoCacheTask.stop(); tlPhotoCacheTask = null; }

  // Run once immediately on startup to evict any entries left over from a previous run
  try {
    const { sweepExpired } = require('./services/memories/tlPhotoCache');
    sweepExpired();
  } catch { /* cache dir may not exist yet — harmless */ }

  tlPhotoCacheTask = cron.schedule('0 */2 * * *', () => {
    try {
      const { sweepExpired } = require('./services/memories/tlPhotoCache');
      sweepExpired();
    } catch (err: unknown) {
